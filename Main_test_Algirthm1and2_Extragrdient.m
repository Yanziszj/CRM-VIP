...
for k=1:3
    ...
    for j=1:3
        ...
        for i=1:Max_experient
            ...
            x0 = Generatingx0Ellipsoid(C,n,m);   
...
%             [xk_Extra,time_Extra,error_Extra,iteration_Extra] =...
%                 Extra_Armijo( C,M,e,Max_iteration3,x0,type_problem);
...
%             Iterations(i,4) = iteration_Yunier_2012;
        ...
        Mid_time_Extra(k,j)  =median(Times(:,3));
...
%         Mid_time_Yunier_2012(k,j)  =median(Times(:,4));
        ...
        Mean_error_Extra(k,j) = mean(Errors(:,3));
...
% Times = log(Times);
PerformanceProfile_time(Time,2);
PerformanceProfile_iteration(Iteration,2);
PerformanceProfile_error(Error,2);
